...
static void nvme_tcp_error_recovery(struct nvme_ctrl *ctrl)
{
	if (nvme_change_ctrl_state(ctrl, NVME_CTRL_FENCING)) {
		...
		queue_work(nvme_reset_wq, &to_tcp_ctrl(ctrl)->fencing_work);
		return;
	}
...
static void nvme_tcp_fencing_work(struct work_struct *work)
{
	...
	queue_delayed_work(nvme_reset_wq, &tcp_ctrl->fenced_work, rem);
	return;
...
static void nvme_tcp_error_recovery_work(struct work_struct *work)
{
	...
	flush_work(&tcp_ctrl->fencing_work);
	flush_delayed_work(&tcp_ctrl->fenced_work);
